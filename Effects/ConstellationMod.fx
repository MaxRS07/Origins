...
float4 ConstellationMod(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float2 node0 = uExtraData.xy * uResolution;
    float2 node1 = uExtraData.zw * uResolution;
    
    float2 tileUV = frac(pcoords / uImageSize0 - screenPosition);
    float3 tex = tex2D(uImage0, tileUV).xyz;

    float2 perp;
    float _ = distanceToLine(pcoords, node0, node1, perp);

    float2 noiseTexture = coords + speed * uTime * 10.;
    float noise = tex2D(uImage1, noiseTexture).x - 0.5;

    float2 distortedCoords = coords + perp * noise * (uOpacity / uResolution);

    float mdist = distanceToLine(distortedCoords * uResolution, node0, node1, perp);
