...
namespace Origins.Items.Weapons.Ranged {
	...
	public class ConstellationNode : ModProjectile {
		...
		public override void Load() { //TODO find out where to load shaders
			//var fillShader = ModContent.Request<Effect>("Origins/Effects/Fill");
			//GameShaders.Misc["Fill"] = new MiscShaderData(fillShader, "Fill");

			var constellation = ModContent.Request<Effect>("Origins/Effects/ConstellationMod");
			GameShaders.Misc["Constellation"] = new MiscShaderData(constellation, "ConstellationMod");
		...
		public List<List<Vector2>> FindTriangles() {
			Dictionary<int, HashSet<int>> adjSet = new();
			for (int i = 0; i < Nodes.Count; i++) {
				adjSet[i] = new HashSet<int>();
			}
			foreach (UnorderedTuple<int> edge in NodeConnections) {
				int u = edge.a;
				int v = edge.b;
				if (u < 0 || u >= Nodes.Count || v < 0 || v >= Nodes.Count) continue;
				if (u == v) continue;
				adjSet[u].Add(v);
				adjSet[v].Add(u);
			}
			...
			foreach (var edge in NodeConnections) {
				...
				int a = Math.Min(u, v);
				int b = Math.Max(u, v);
				(int a, int b) edgeTuple = (a, b);
				if (processedEdges.Contains(edgeTuple)) continue;
				processedEdges.Add(edgeTuple);
				foreach (int c in adjSet[a]) {
					if (c <= b) continue;
					if (adjSet[b].Contains(c)) {
						triangles.Add(new List<int> { a, b, c });
					}
				}
			}
			return triangles.Select(o => o.Select(o2 => Nodes[o2].Center).ToList()).ToList();
		}
		...
		private float Progress => MathF.Min(Projectile.frameCounter / 30f, 1f);

		private float DrawWidth => MathHelper.Clamp((Projectile.frameCounter - 20) / 15f, 0f, 1f);
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
			if (GroupRoot) {
				foreach (var c in NodeConnections) {
					...
					if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), start, end, 20, ref _)) {
						return true;
					}
				}
			}
